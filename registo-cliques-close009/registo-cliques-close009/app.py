...
def export_csv():
    """Export all clicks to a CSV file with semicolon delimiter."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
...
def export_txt():
    """Export all clicks to a plain text file."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT button_label, click_number, click_date, click_time FROM clicks ORDER BY click_date DESC, click_number DESC")
    rows = cursor.fetchall()
    ...
    output = io.StringIO()
    output.write("REGISTO DE CLIQUES - LOJA DO CIDADÃO\n")
    output.write("=" * 40 + "\n\n")
    output.write(f"{'Serviço':<20} | {'Nº':<5} | {'Data':<12} | {'Hora'}\n")
    output.write("-" * 60 + "\n")
    
    for row in rows:
        output.write(f"{row[0]:<20} | {row[1]:<5} | {row[2]:<12} | {row[3]}\n")
    ...
    return send_file(
        ...
        mimetype="text/plain",
        as_attachment=True,
        download_name="registo_cliques.txt"
    )
